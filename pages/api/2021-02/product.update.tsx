  ...
  try {
    ...
    await db.collection("product.update").doc(product_id).get().then((doc) => {
      ...
      if (doc.exists) { // 30 seconds ago
        if (doc.data().v_created_at > Date.now() - 45 * 1000) {
          console.log(`wait: ${Math.floor(((Date.now() - 45 * 1000) - doc.data().v_created_at) / -1000)}s`);

          duplicate = true;
        }
        if (doc.data().s_created_at > Date.now() - 60 * 2 * 1000) {
          console.log(`wait: ${Math.floor(((Date.now() - 60 * 2 * 1000) - doc.data().s_created_at) / -1000)}s`);
          duplicate = true;
        }
    ...
    if (duplicate) {
      console.log(Date.now() - prevTimer);
      console.log(`id: ${handle} - Already processing - vend`);
