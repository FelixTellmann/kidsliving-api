...
export default async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  ...
  function getMorePromise(i, sinceDate) {
    const promise = axios(
      `http://${req.headers.host}/api/getProductSalesCustomer?since=${sinceDate}&page=${i}&page_size=200`
    );
    return promise;
  }
  ...
  if (!page) {
    sales = sales.reduce(
      (
        acc: CustomerData[],
        { register_sale_products, sale_date, customer: { name: customer_name, email, phone, mobile }}
      ) => {
