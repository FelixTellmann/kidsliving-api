...
export default async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  ...
  if (shopifyWebhook) {
    ...
    } = await getShopifyFulfillmentOrders(order_id);
    ...
    if (!possibleChange) {
      await db
        .collection("fulfillment_orders")
        .doc("" + order_id)
        .set({ fulfillment_orders: fulfillment_orders.map(({ id }) => id) });
      res.status(200).json("New Order / no fulfillment changes");
      return;
    }

    const sorted_fulfillment_orders: VariantSort = fulfillment_orders.reduce(
      (acc, { id, assigned_location_id, status, line_items }) => {
        line_items.forEach(({ variant_id, quantity }) => {
          if (acc[variant_id]) {
            acc[variant_id].push({
              ...
              status,
              quantity,
              needsProcessing: !previouslyProcessed.includes(id),
            });
          } else {
            acc[variant_id] = [];
            acc[variant_id].push({
        ...
        acc[key] = val.reduce(
          (acc, itm) => {
            if (!itm.needsProcessing) {
              ...
              if (
                !(
                  acc.lastProcessed.quantity === itm.quantity &&
                  acc.lastProcessed.assigned_location_id === itm.assigned_location_id
                )
              ) {
