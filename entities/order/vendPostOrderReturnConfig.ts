...
export const postNewVendOrderReturnConfig: TPostNewVendOrder = (
  shopifyOrderWebhook,
  vendProducts,
  vendShippingProduct,
  vendCustomer,
  ...
  const orderConfig: TPostOrder = {
    ...
    register_sale_products: [
      ...shopifyOrderWebhook.line_items.map((line_item) => {
        ...
        const matchingFulfillment = fulfillmentOrders.data.fulfillment_orders.find((fulfillmentOrder) => {
          ...
          if (fulfillmentOrder.line_items.some(({ line_item_id }) => line_item.id === line_item_id)) { /* FIXME: Hacky way to find last item in array.... */
        ...
        return {
          ...
          register_id: `${matchingFulfillment.assigned_location_id}` === SHOPIFY_CPT_OUTLET_ID
            ? VEND_REGISTER_CPT_TILL2_ID
            : VEND_REGISTER_JHB_TILL2_ID,
          ...
          tax: line_item.taxable ? +(Math.round(+price / 1.15 * 0.15).toPrecision(2)) : 0.00,
    ...
    register_sale_payments: shopifyOrderWebhook.financial_status === 'paid' ? [
      {
        retailer_payment_type_id: shopifyOrderWebhook.gateway === "paygate"
          ? VEND_PAYMENT_CC_ID
          : VEND_PAYMENT_EFT_ID,
